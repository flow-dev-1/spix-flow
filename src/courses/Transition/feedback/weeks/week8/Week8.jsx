...
function Week8({ enrollmentId, setWeekEightData, isSchool, studentId }) {
  ...
  const { questions: assessments } = getWeekAssessment(8);

  // toDo: Fetch User assessment and Activity Data
  const { data, isPending, status, isError } = useQuery({
    queryKey: ["dashboard/transition-feedback-8", enrollmentId, 8],
    queryFn: () => {
      if (isAdmin) return adminService.getUserCourseData(enrollmentId, 8, code);
      if (isSchool) return schoolService.getStudentCourseData(enrollmentId, 8, studentId);
      return userService.getUserCourseData(enrollmentId, 8);
    },
    ...
    <>
      ...
      <div className="d-flex gap-3">
        ...
        <p className="text-blue fs-md-4">
          {activity2.question} "Goal Setting"?
        </p>
      </div>
      <div className="d-flex gap-3">
        <h2 className="text-gray fs-md-1 text-gray">Answers:</h2>
