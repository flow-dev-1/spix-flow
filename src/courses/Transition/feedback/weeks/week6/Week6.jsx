...
function Week6({ enrollmentId, setWeekSixData, isSchool, studentId }) {
  ...
  const { data, isPending, status, isError } = useQuery({
    ...
    queryFn: () => {
      if (isAdmin) return adminService.getUserCourseData(enrollmentId, 6, code);
      if (isSchool) return schoolService.getStudentCourseData(enrollmentId, 6, studentId);
      return userService.getUserCourseData(enrollmentId, 6);
    },
  ...
  function drag1(type) {
    ...
    const indices =
      type === "growth"
        ? activityData[4].answer.green
        : activityData[4].answer.red;

    return indices?.map((index) => activity5?.images[index]) || [];
  }
    ...
    <>
      ...
      <div className="d-flex gap-3">
        ...
        <p className="text-blue fs-md-4">
          {activity1.question} "Social Skills"?
        </p>
      </div>
      <div className="d-flex gap-3">
      ...
      <div className="d-flex gap-3">
        <h2 className="text-blue fs-md-1">Questions:</h2>
        <p className="text-blue fs-md-4">
          {activity3.question} "Communcation"?
        </p>
      </div>
      <div className="d-flex gap-3">
        <h2 className="text-gray fs-md-1 text-gray">Answers:</h2>
      ...
      <div className="d-flex gap-3">
        <h2 className="text-blue fs-md-1">Questions:</h2>
        <p className="text-blue fs-md-4">{activity4.question} "Boundaries"?</p>
      </div>
      <div className="d-flex gap-3">
        <h2 className="text-gray fs-md-1 text-gray">Answers:</h2>
      ...
      <div className="d-flex gap-3">
        <h2 className="text-blue fs-md-1">Questions:</h2>
        <p className="text-blue fs-md-4">{activity5.instruction}</p>
      </div>
      <div className="d-flex gap-3">
        <h2 className="text-gray fs-md-1 text-gray">Answers:</h2>
        <div className="flex-grow-1 d-flex">
          <div className="flex-grow-1">
            ...
            <div className="px-md-5 px-2 py-md-3 py-1">
      ...
      <div className="d-flex gap-3">
        <h2 className="text-blue fs-md-1">Questions:</h2>
        <p className="text-blue fs-md-4">
          Below are sentences with missing words related to communication and
          boundaries. Choose the best word from the list below to complete each
          sentence.
        </p>
      </div>
      <div className="d-flex gap-3">
        ...
        <ul className="list-unstyled fs-md-5 flex-grow-1">
          {activity6.steps.slice(1).map((step, idx) => {
            const answer =
              getActivityAnswer(activity6.id)?.find(
                (ans) => ans.stepId === step.stepId
              )?.value || "No answer";

            return (
              <li key={idx} className="fs-md-5 mb-3">
                {idx + 1}. {step.question} (<b>{answer}</b>)
              </li>
            );
          })}
        </ul>
          ...
          <>
            ...
            {options.map((option, index) => {
              ...
              return (
                <div
                  key={index}
                  className="d-flex gap-1 p-md-2 mb-3 justify-content-between"
                >
