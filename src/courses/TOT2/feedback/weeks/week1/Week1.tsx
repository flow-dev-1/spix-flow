...
function Week1({ enrollmentId, setWeekOneData }) {
  ...
  const { questions } = getWeekAssessment(1);
  ...
  const { data, isPending, status, isError } = useQuery({
    queryKey: ["dashboard/tot-feedback-2", enrollmentId, 1],
    queryFn: () =>
      isAdmin
  ...
  useEffect(() => {
    ...
    setPreAssessmentData(
      data.activity?.activities.find((p) => p.page == 1).answer,
    );
  ...
  function drag1(type) {
    ...
    const indices =
      type === "inclusion"
        ? activityData[4]?.answer?.[0]?.value?.green
        : type === "integration"
          ? activityData[4]?.answer?.[0]?.value?.orange
          : activityData[4]?.answer?.[0]?.value?.red;
    return indices?.map((index) => activity4?.steps?.[1].images[index]) || [];
  }

  function drag2(type) {
    console.log(activityData, "Activity Data");
    if (!activityData || !activityData[8] || !activityData[8].answer) return [];

    const indices =
      type === "equality"
        ? activityData[8]?.answer?.[0]?.value?.green
        : activityData[8]?.answer?.[0]?.value?.orange;

    return indices?.map((index) => activity8?.steps?.[1].images[index]) || [];
  }
  ...
  return (
    <>
      ...
      <div className="d-flex gap-3">
        ...
        <div className="flex-grow-1 d-flex">
          ...
          <div className="flex-grow-1">
            ...
            <div className="px-2 py-1 px-md-5 py-md-3">
              {drag1("integration")?.map((item, idx) => (
                <p className="fs-md-4">
                  {idx + 1}. {item}
                </p>
              ))}
          ...
          <div className="flex-grow-1">
            ...
            <div className="px-2 py-1 px-md-5 py-md-3">
              {drag1("Segregatiion")?.map((item, idx) => (
                <p className="fs-md-4">
                  {idx + 1}. {item}
                </p>
              ))}
      ...
      <hr />
      <span className="text-gray">{activity7.steps[0].text}</span>
      <br />
      <div className="d-flex gap-3">
        <p
          className="text-blue fs-md-1 week-2-question-text fw-bold"
      ...
      <div className="d-flex gap-3">
        ...
        <div>
          {(() => {
            ...
            const step = answers.find(
              (a) => a.stepId === activity7.steps[2].stepId,
            );
            if (!step || !step.value)
              return <p className="text-gray">Loading...</p>;

            const details = getOptionDetails(
              activity7.steps[2].options,
              step.value,
            );
            if (!details) return <p className="text-gray">Loading...</p>;
      ...
      <hr />
      <span className="text-gray">{activity7.steps[3].text}</span>
      <br />
      <div className="d-flex gap-3">
        <p
          className="text-blue fs-md-1 week-2-question-text fw-bold"
      ...
      <div className="d-flex gap-3">
        ...
        <div>
          {getAllOptionsWithCorrectAnswer(activity7.steps[5].options, 1)}
        </div>
      ...
      <div className="d-flex gap-3">
        ...
        <div>
          {(() => {
            ...
            const step = answers.find(
              (a) => a.stepId === activity7.steps[6].stepId,
            );
            if (!step || !step.value)
              return <p className="text-gray">Loading...</p>;

            const details = getOptionDetails(
              activity7.steps[6].options,
              step.value,
            );
            if (!details) return <p className="text-gray">Loading...</p>;
      ...
      <hr />
      <span className="text-gray">{activity7.steps[7].text}</span>
      <br />
      <div className="d-flex gap-3">
        <p
          className="text-blue fs-md-1 week-2-question-text fw-bold"
      ...
      <div className="d-flex gap-3">
        ...
        <div>
          {(() => {
            ...
            const step = answers.find(
              (a) => a.stepId === activity7.steps[8].stepId,
            );
            if (!step || !step.value)
              return <p className="text-gray">Loading...</p>;

            const details = getOptionDetails(
              activity7.steps[8].options,
              step.value,
            );
            if (!details) return <p className="text-gray">Loading...</p>;
